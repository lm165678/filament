...
bool VulkanDriver::getTimerQueryValue(Handle<HwTimerQuery> tqh, uint64_t* elapsedTime) {
    ...
    // Note that the client (FrameInfo) rotates its own set of high-level query object at every
    // frame. i.e. it never uses the same high-level query object on consecutive frames.
    ...
    ASSERT_POSTCONDITION(results[1] >= results[0], "Timestamps are not monotonic.");
